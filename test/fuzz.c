...
static uint16_t read_uint16le(uint8_t const *buffer) {
  return (uint16_t)buffer[0] | (uint16_t)buffer[1] << 8;
}

static uint16_t read_uint16be(uint8_t const *buffer) {
...
#define UTF16_HELPERS(endianness)                                              \
  ...
  static bool is_valid_utf16##endianness(const uint8_t *buffer, size_t len,    \
    ...
    return true;                                                               \
                                                                               ...
                                                                               \
  /* Generated by Claude */                                                    \
  static void print_code_points_utf16##endianness(char const *s, size_t len) { \
    if (s == NULL) {                                                           \
      return;                                                                  \
    }                                                                          \
                                                                               \
    ...
    while (i + 1 < len) {                                                      \
      ...
      uint16_t cu = read_uint16##endianness((uint8_t const *)s + i);           \
      i += 2;                                                                  \
                                                                               \
      uint32_t code_point;                                                     \
                                                                               \
      ...
      if (cu >= 0xD800 && cu <= 0xDBFF) {                                      \
        ...
        if (i + 1 >= len) {                                                    \
          ...
          break;                                                               \
                                                                               ...
                                                                               \
        uint16_t cu2 = read_uint16##endianness((uint8_t const *)s + i);        \
        i += 2;                                                                \
                                                                               \
...
// Generated by Claude
static void print_code_points_utf8(char const *s, size_t len) {
  if (!s)
    return;
